...
if {$DCRM_ELABORATED_DESIGN_DDC_INPUT_FILE == ""} {
if {[file exists [which ${RTL_SOURCE_FILES_SV}]]} {
  puts "RM-Info: Reading sverilog files from [which ${RTL_SOURCE_FILES_SV}]\n"
  set filelist [read [ open ${RTL_SOURCE_FILES_SV} r]]
  set filelist $filelist
  analyze -format sverilog ${filelist}
}
if {[file exists [which ${RTL_SOURCE_FILES_VLOG}]]} {
  puts "RM-Info: Reading verilog files from [which ${RTL_SOURCE_FILES_VLOG}]\n"
  set filelist [read [ open ${RTL_SOURCE_FILES_VLOG} r]]
  set filelist [regsub -all "..MODULES_PATH." $filelist $env(MODULES_PATH) ]
  analyze -format verilog ${filelist}
}
if {[file exists [which ${RTL_SOURCE_FILES_VHDL}]]} {
  puts "RM-Info: Reading vhdl files from [which ${RTL_SOURCE_FILES_VHDL}]\n"
  set filelist [read [ open ${RTL_SOURCE_FILES_VHDL} r]]
  set filelist   $filelist
  analyze -format vhdl ${filelist}
}
...
read_ddc ${DCRM_ELABORATED_DESIGN_DDC_INPUT_FILE}
set_verification_top
}
...
if {![file isdirectory icc2_workdir]} {
sh mkdir icc2_workdir
}
set_icc2_options -work_dir icc2_workdir -keep_files
...
#################################################################################
# Below section runs non-scan synthesis
#################################################################################

if {$NO_AUTOUNGROUP == TRUE} { 
compile_ultra -gate_clock -spg -no_autoungroup
} else {
...
#### Added to uniquify the naming style of tico & axis libraires with desig name as prefix
...
#Script to dump qor summary with frequency feasibility
source ./rm_dc_scripts/proc_qor.tcl
proc_qor -existing_qor_file ${REPORTS_DIR}/${DCRM_FINAL_QOR_REPORT} > ${REPORTS_DIR}/QOR_SUMMARY.rpt
...
# Utility to report the area details for different hierarchies in the design
source rm_dc_scripts/procedures.tcl
source rm_dc_scripts/area_reporting.tcl
create_design_data dataArray 2 0
report_design_data dataArray 2 20 > ${REPORTS_DIR}/area_hier.csv
...
# Creates abstract view of the design & dumps out abstract ddc for top level synthesis
create_block_abstraction
write -format ddc -hierarchy -output ${RESULTS_DIR}/${DESIGN_NAME}.mapped.abstract.ddc

if {$REPORT_CONGESTION == TRUE} {
if {[shell_is_in_topographical_mode]} {
